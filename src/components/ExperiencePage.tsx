    ...
    <div className="bg-ivory text-charcoal">
      ...
      <section className="bg-ivory">
        <div className="mx-auto max-w-[900px] px-6 py-20 md:px-10 md:py-28">
          ...
          <p className="mt-5 text-lg leading-relaxed text-charcoal/80 text-pretty">
            {experience.intro}
          </p>
        </div>
      </section>
              ...
              <table className="w-full min-w-[560px] border-collapse text-left text-sm">
                <thead>
                  <tr className="border-b border-ivory/10">
                    <th className="px-6 py-5 text-xs uppercase tracking-widest text-ivory/50">
                      &nbsp;
                    </th>
                    <th className="px-6 py-5 font-display text-lg text-gold">
                      {experience.comparison.labelA}
                    </th>
                    <th className="px-6 py-5 font-display text-lg text-gold">
                      {experience.comparison.labelB}
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {experience.comparison.rows.map((r) => (
                    <tr key={r.label} className="border-b border-ivory/10 last:border-0">
                      <td className="px-6 py-4 text-xs uppercase tracking-widest text-ivory/50">
                        {r.label}
                      </td>
        ...
        <div className="mx-auto max-w-[900px] px-6 py-20 md:px-10 md:py-28">
          <div className="space-y-14">
            {experience.sections.map((s) => (
              <div key={s.title}>
                <h2 className="font-display text-2xl text-charcoal text-balance md:text-3xl">
                  {s.title}
                </h2>
                <p className="mt-4 text-base leading-relaxed text-charcoal/75 text-pretty">
                  {s.body}
                </p>
              </div>
            ))}
          ...
          {experience.journey && (
            ...
            >
              <div>
                ...
                <div className="mt-2 font-display text-xl text-charcoal">
                  {experience.journey.name}
                </div>
              </div>
          ...
          <div className="mt-8 grid grid-cols-1 gap-6 sm:grid-cols-3">
            {others.map((e) => (
              ...
              >
                ...
                <div className="absolute inset-x-4 bottom-4 font-display text-lg text-ivory">
                  {e.title}
                </div>
              </Link>
            ))}
